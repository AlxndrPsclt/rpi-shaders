...
void main()
{
    ...
    for(float i = 0.0; i < 7.0; ++i) {
      possibleToShowWave = (smoothstep(0.8,0.9,noise(7.333*(i+1.0)*time))+0.05);
      ...
      lineSDF = sdSegment(uv + onde, vec2(0.0,fract(time/10.0+abs(sin((i+ctime)/6.0)))), vec2(1.0,abs(sin((i+ctime)/6.0))));
...
//    nline += 1.0-smoothstep(0.002,0.0021,abs(lineSDF6));
    ...
    color+=nline*vec3(2.0*smoothLowfreqs2,0.1,0.1)*1.5;
    ...
    //float wanderingLineSDF = sdSegment(uv, vec2(0.0,abs(sin(ctime/2.0))), vec2(1.0,abs(sin(ctime/2.0))));
    float wanderingLine=0.0;
    float wanderingLineSDF = sdSegment(uv, vec2(0.0,sin(time*0.1)), vec2(1.0,cos(time*0.1)));
    wanderingLine += 1.0-smoothstep(0.004,0.0045,abs(wanderingLineSDF));
    vec3 wanderingLineColored = wanderingLine * vec3(0.0,0.5,0.1)*(1.0-smoothLowfreqs2);

    color+=wanderingLineColored;
