...
void main()
{
    ...
    float possibleToShowWave = (smoothstep(0.2,0.4,noise(time))+0.05);
    float smoothLowfreqs2 = smoothstep(1.0,5.0,lowfreqs*possibleToShowWave);
    ...
    //
    float ctime = time*0.1+smoothLowfreqs2;
    ...
    for(float i = 0.0; i < 10.0; ++i) {
      onde = vec2(0.0, cos(uv.x*3.33*PI-fract(i*fastime))*sin(uv.x*10.0*i*PI-(i*fastime)/12.5)*smoothLowfreqs2;
