...
void main()
{
    vec2 uv = 7.0*gl_FragCoord.xy/resolution;
    ...
    float grid = drawGrid(uv+vec2(0.0, noise(sin(time/5.0))*10.0*noise(time/17.77)*noise(noise(time/6.0)*uv.x*noise(uv.x*uv.y)*0.5*floor(uv.y)*noise(50.0*uv.y))), vec2(0.0,cos(noise(time))));
    ...
    gl_FragColor = vec4(30.0*color, 1.0);
}
